...
// SACA TODOSLOS ELEMENTOS DEL OBJETO CON ESAS PROPIEDADES

/* for (let i = 0; i < arrayPersonas.length; i++) {
  cardPersona += ` <div class="card-persona">
  <img src="${personas[i].picture}"  alt="">
    <p>"${personas[i].name}"</p>
    <p>"${personas[i].age}"</p>
    <p>"${personas[i].email}"</p>
     <p>"${personas[i].addres}"</p>
  </div>
  `;
}
 */
/* document.getElementById("container").innerHTML = cardPersona;

// SACA TODOSLOS UN ELEMENTO (EL PRIMERO) DEL OBJETO CON ESAS PROPIEDADES

for (let i = 0; i < arrayPersonas.length; i++) {
  cardPersona = ` <div class="card-persona">
  <img src="${personas[0].picture}"  alt="">
    <p>"${personas[0].name}"</p>
    <p>"${personas[0].age}"</p>
    <p>"${personas[0].email}"</p>
     <p>"${personas[0].addres}"</p>
      <p>"${personas[0].eyeColor}"</p>
  </div>
  `;
}
...
document
  ...
  .style.backgroundColor(personas[0].eyeColor); */

/* Para sacar una persona, segun su posición */
...
function buscarPersonas() {
  ...
  for (valor = 1; i < arrayPersonas.length; valor++) {
    mostrarPersonas = ` <div class="card-persona">
  <img src="${personas[valor].picture}"  alt="">
    <p>"${personas[valor].name}"</p>
    <p>"${personas[valor].age}"</p>
    <p>"${personas[valor].email}"</p>
     <p>"${personas[valor].addres}"</p>
      <p>"${personas[valor].eyeColor}"</p>
  </div>
  `;
